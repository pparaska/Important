...
public class BankAppController {
	...
	public String deposit(@ModelAttribute Transaction transaction, Model model) {
		restTemplate.postForEntity("http://localhost:8787/TransactionService/transactions", transaction, null);
	...
	public String fundTransfer(@RequestParam("sendersAccountNumber") int sendersAccountNumber, @RequestParam("receiversAccountNumber") int receiversAccountNumber,
		...
		restTemplate.postForEntity("http://localhost:8787/TransactionService/transactions/withdraw", sendersTransaction, null);
		Transaction receiversTransaction = new Transaction(receiversAccountNumber, amount);
		restTemplate.postForEntity("http://localhost:8787/TransactionService/transactions", receiversTransaction, null);
		model.addAttribute("message","Success!");
		return "FundTransfer";
	}
	...
	public String withdraw(@ModelAttribute Transaction transaction, Model model) {
		restTemplate.postForEntity("http://localhost:8787/TransactionService/transactions/withdraw", transaction, null);
		model.addAttribute("message","Success!");
		return "Withdraw";
	}
	...
	public ModelAndView getStatement(@RequestParam("offset") int offset, @RequestParam("size") int size) {
		...
		CurrentDataSet currentDataSet = restTemplate.getForObject("http://localhost:8787/TransactionService/transactions", CurrentDataSet.class);
